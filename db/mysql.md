...
### 普通索引(不包括唯一索引)
```
加锁规则和唯一差不多，不过除了在普通索引加锁外，还要在扫描出来的加主键加锁
```
### 没有走索引的情况
```
对所有记录都加锁了，相当于加了表级锁
...
什么是redo log
```
redo log是物理日志，记录了xx表空间中的xx数据页偏移量(某个物理地址)做了AA更新。执行一个事务就会产生一条或多条这样的日志
```
WAL技术
```
即先写到缓存，然后后台线程再慢慢刷到磁盘(当然有参数配置，可以控制同步刷到磁盘)
```
redo log + WAL的作用
```
mysql服务崩溃后，事务恢复能力，即是crash safe 能力
随机io变到顺序io
```
redo log 写满后，会怎么样？
